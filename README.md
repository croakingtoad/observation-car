...
3. Pick the version you want — a GitHub release must already exist for that version; BRAT installs the release's top-level assets (`main.js`, `manifest.json`, `styles.css`). `versions.json` is also published on the release for Obsidian's community-plugin update flow.
