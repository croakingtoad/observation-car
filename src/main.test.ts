...
describe("Booklore registrations from plugin onload", () => {
  ...
  it("registers the Booklore catalog during onload", async () => {
    ...
    const commands = (plugin as unknown as { commands: Command[] }).commands;
    expect(
      commands.some((command) => command.id === "browse-booklore-catalog"),
    ).toBe(true);
  });

  it("registers the Booklore downloader during onload", async () => {
    const { getBookloreDownloader } = await import(
      "./booklore/bookDownloadRegistration"
    );
    const plugin = makePlugin();

    await plugin.onload();

    expect(getBookloreDownloader(plugin)).toBeDefined();
  });
